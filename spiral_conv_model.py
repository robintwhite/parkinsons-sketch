...
df_train_images = pd.DataFrame({'path': list(data_dir.glob(r'spiral/training/*/*.png'))})
...
spiral_datagen = ImageDataGenerator(rotation_range=90,
...
spiral_train_generator = spiral_datagen.flow_from_dataframe(df_train_images,
                                                            x_col='path',
                                                            y_col='label',
                                                            subset="training",
                                                            ...
                                                            shuffle=True,
                                                            seed=42)
spiral_val_generator = spiral_datagen.flow_from_dataframe(df_train_images,
                                                            x_col='path',
                                                            y_col='label',
                                                            subset="validation",
                                                            target_size=(img_height, img_width),
                                                            ...
                                                            shuffle=True,
                                                            seed=42)
class_names = [key for key in spiral_train_generator.class_indices.keys()]
print(class_names)

#visualize augmentations
# configure batch size and retrieve one batch of images
# for X_batch, y_batch in spiral_val_generator:
...
opt = Adam(1e-4)#Nadam(lr=3e-4)
model = SpiralNet.build(width=img_width, height=img_height, depth=1,
                        classes=1)
...
model.fit_generator(spiral_train_generator,
...
# random.shuffle(df_train_images) # returns in place
# NUM_TRAIN_IMAGES = len(df_train_images)
# print(f'[INFO] number of training images: {NUM_TRAIN_IMAGES}')
# df_test_images = list(data_dir.glob(r'spiral/testing/*/*.png'))
# random.shuffle(df_test_images)
# NUM_TEST_IMAGES = len(df_test_images)
# print(f'[INFO] number of test images: {NUM_TEST_IMAGES}')
# imagePaths = df_train_images + df_test_images
